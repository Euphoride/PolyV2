...
var modifyTree = function (key, value, overview) {
    ...
    var _ = key[0], newKey = key.slice(1);
    var newChildren = overview.children.map(function (child) {
        return child.key === key[0] ? modifyTree(newKey, value, child) : child;
    });
...
var deleteInTree = function (key, overview) {
    ...
    var newChildren = filteredChildren.map(function (child) {
        return deleteInTree(newKey, child);
    });
