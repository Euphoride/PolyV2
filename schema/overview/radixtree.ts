...
const findInTree = <A, B>(key: A[], overview: RadixTree<A, B>): B => {
	...
	const mostSpecificChild = overview.children.find(
		(child) => child.key === key[0]
	);
...
const modifyTree = <A, B>(
	...
	const newChildren = overview.children.map((child) =>
		child.key === key[0] ? modifyTree(newKey, value, child) : child
	);
...
const deleteInTree = <A, B>(
	...
	const filteredChildren = overview.children.filter(
		(child) => !(key.length === 1 && key[0] === child.key)
	);
	const newChildren = filteredChildren.map((child) =>
		deleteInTree(newKey, child)
	);
